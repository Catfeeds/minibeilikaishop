      ...
      <table class="pro_3">
         ...
         <!-- 遍历 -->
          <?php if(is_array($productlist)): $i = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
             <td><?php echo ($v["id"]); ?></td>
             <td style="padding:3px 0;"><img src="/minigueinon/Data/<?php echo ($v["photo_x"]); ?>" width="80px" height="80px"/></td>
             <td><?php echo ($v["shangchang"]); ?></td>
             <td><?php echo ($v["name"]); ?></td>
             <td><?php if($v["attr"] != ''): echo ($v["attr_name"]); ?>:<?php echo ($v["attr"]); else: ?>无<?php endif; ?></td>
             <td><?php if($v["spec"] != ''): echo ($v["spec_name"]); ?>:<?php echo ($v["spec"]); else: ?>无<?php endif; ?></td>
             <td><?php echo ($v["price_yh"]); ?></td>
             <td><?php echo ($v["renqi"]); ?></td>
             <td><a href="<?php echo U('set_new');?>?pro_id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&name=<?php echo ($name); ?>"><?php if($v["is_show"] == 1): ?><label style="color:green;">是</label><?php else: ?><label style="color:gray;">否</label><?php endif; ?></a></td>
