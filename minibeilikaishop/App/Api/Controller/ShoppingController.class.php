...
class ShoppingController extends PublicController {
	...
	public function index(){
		...
		$cart = $shopping->where('uid='.intval($user_id))->field('id,uid,pid,price,num,shop_id')->select();
        foreach ($cart as $k => $v) {
        	...
        	$cart[$k]['shop_name'] = M('shangchang')->where('id="'.$v['shop_id'].'"')->getField('name');
        }

        // $result = array();
        // foreach($cart as $k => $v){
        // 	$result[$v['shop_id']][$k] = $v;
        // }
        // $cart = $result;
