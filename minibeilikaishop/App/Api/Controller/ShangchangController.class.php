...
class ShangchangController extends PublicController {
    ...
    public function shop_details(){
    	...
    	$shop_info = M('shangchang')->where('id='.intval($shop_id))->field('id,name,uname,tel,logo,address,content,grade')->find();
    	if (!$shop_info) {
    	...
    	//获取8个商品
    	$pro_list = M('product')->where('shop_id='.intval($shop_id).' AND del=0 AND is_down=0 AND pro_type=1')->order('addtime desc,sort desc')->field('id,name,intro,price_yh,photo_x,shiyong')->limit(20)->select();
        $salenum = 0;
    	foreach ($pro_list as $k => $v) {
    		$pro_list[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            $pro_list[$k]['price_yh'] = M('attr_spec_price_store')->where('pid="'.intval($v['id']).'"')->getField('price_yh');
            $salenum += $v['shiyong'];
    	}
        $shop_info['salenum'] = $salenum;

        //获取商家推荐的商品
        $tj_list = array();
        $tj_list = M('product')->where('shop_id='.intval($shop_id).' AND del=0 AND is_down=0 AND pro_type=1 AND type=1')->order('addtime desc,sort desc')->field('id,name,intro,price_yh,photo_x,shiyong')->limit(20)->select();
        $salenum = 0;
        foreach ($tj_list as $k => $v) {
            $tj_list[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            $tj_list[$k]['price_yh'] = M('attr_spec_price_store')->where('pid="'.intval($v['id']).'"')->getField('price_yh');
            $salenum += $v['shiyong'];
        }
        $tj_info['salenum'] = $salenum;

        //获取一周内的新品
        $list = array();
        $list = M('product')->where('shop_id='.intval($shop_id).' AND del=0 AND is_down=0 AND pro_type=1 AND addtime >"'.(time()-strtotime('-7 days')).'"')->order('addtime desc,sort desc')->field('id,name,intro,price_yh,photo_x,shiyong')->limit(20)->select();
        $salenum = 0;
        foreach ($list as $k => $v) {
            $list[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            $list[$k]['price_yh'] = M('attr_spec_price_store')->where('pid="'.intval($v['id']).'"')->getField('price_yh');
            $salenum += $v['shiyong'];
        }
        $tj_info['salenum'] = $salenum;
    	echo json_encode(array('status'=>1,'shop_info'=>$shop_info,'pro'=>$pro_list,'tj_list'=>$tj_list,'list'=>$list));
    	exit();
    }
    /**
