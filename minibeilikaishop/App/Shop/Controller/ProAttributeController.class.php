...
class ProAttributeController extends PublicController{
	...
	*/
	public function index(){
		...
		//分页
		$count   = $this->Attribute->where('attr_name LIKE "%'.$attr_name.'%" AND shop_id="'.$shop_id.'" OR shop_id=0 AND attr_name LIKE "%'.$attr_name.'%"')->count();// 查询满足要求的总记录数
		...
		$attr_list = $this->Attribute->where('attr_name LIKE "%'.$attr_name.'%" AND shop_id="'.$shop_id.'" OR shop_id=0 AND attr_name LIKE "%'.$attr_name.'%"')->limit($Page->firstRow.','.$Page->listRows)->select();	
	...
	*/
	public function add(){
		...
		if (intval($_GET['attr_id'])) {
			//判断是否有权利删除
			$shop_id = intval($_SESSION['shopinfo']['shop_id']);
			$attr_shop_id = M('attribute')->where('id="'.$id.'"')->getField('shop_id');
			if($attr_shop_id != $shop_id || $attr_shop_id == 0){
