...
class ProSpecController extends PublicController{
	...
	*/
	public function index(){
		...
		//分页
		$count   = $this->spec->where('spec_name LIKE "%'.$spec_name.'%" AND shop_id="'.$shop_id.'" OR shop_id=0 AND spec_name LIKE "%'.$spec_name.'%"')->count();// 查询满足要求的总记录数
		...
		$spec_list = $this->spec->where('spec_name LIKE "%'.$spec_name.'%" AND shop_id="'.$shop_id.'" OR shop_id=0 AND spec_name LIKE "%'.$spec_name.'%"')->limit($Page->firstRow.','.$Page->listRows)->select();	
	...
	*/
	public function add(){
		...
		if (intval($_GET['spec_id'])) {
			...
			//判断是否有权利删除
			$shop_id = intval($_SESSION['shopinfo']['shop_id']);
			$spec_shop_id = M('spec')->where('id="'.$spec_id.'"')->getField('shop_id');
			if($spec_shop_id != $shop_id || $spec_shop_id == 0){
				$this->error('该属性属于超级管理员管理！');
			}
	...
	*/
	public function del(){
		...
		//判断是否有权利删除
		$shop_id = intval($_SESSION['shopinfo']['shop_id']);
		$spec_shop_id = M('spec')->where('id="'.$id.'"')->getField('shop_id');
		if($spec_shop_id != $shop_id || $spec_shop_id == 0){
